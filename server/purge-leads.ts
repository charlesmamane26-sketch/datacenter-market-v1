...
async function main(): Promise<void> {
  ...
  const { anonymized, deleted } = await purgeLeadsOlderThan(retentionDays);
  console.log(
    `[Purge] Removed ${deleted} unconverted lead(s) and anonymised ${anonymized} referenced lead(s) older than ${retentionDays} days.`
  );
}
